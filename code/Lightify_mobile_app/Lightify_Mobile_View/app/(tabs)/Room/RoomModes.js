...
export default function RoomModesScreen() {
  ...
  // ✅ Fetch modes from /api/rooms/configure
  useEffect(() => {
    const fetchRoomModes = async () => {
      try {
        const res = await axiosClient.get(
          `/api/rooms/configure?username=Tharindu&roomName=${roomName}`
        );
        const data = res.data;

        if (Array.isArray(data.Automation_Modes)) {
          const extractedModes = data.Automation_Modes.map((mode) => ({
            _id: mode.Mode_Name.toLowerCase().replace(/\s+/g, "_"),
            name: mode.Mode_Name,
            active: false, // default as inactive
          }));
