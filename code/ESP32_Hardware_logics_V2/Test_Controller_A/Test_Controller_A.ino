...
// ——— Utility: print task stack watermark ———
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    MQTTHandler::onMessage(handleMqtt);
    Serial.println("MQTT Successfully Handled!");
  } else {
...
void loop() {
  ...
  // —— Core tasks ——
  // Serial.println(F("[LOOP] -> SerialComm::loop()"));
  SerialComm::loop();
  // Serial.println(F("[LOOP] <- SerialComm::loop()"));
  if (WiFi.status() == WL_CONNECTED){
    // Serial.println(F("[LOOP] -> MQTTHandler::loop()"));
    MQTTHandler::loop();
    // Serial.println(F("[LOOP] <- MQTTHandler::loop()"));
  }
  // MQTTHandler::loop();
  // Serial.println(F("[LOOP] -> LightManager::update()"));
  LightManager::update();
  // Serial.println(F("[LOOP] <- LightManager::update()"));
