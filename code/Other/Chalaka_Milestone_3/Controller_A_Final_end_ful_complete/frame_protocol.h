...
// Packs “payload” into outBuf as: [0x7E][lenH][lenL][payload…][checksum]
// Returns false if payload too large.
...
// Stateful parser for one frame at a time
class FrameParser {
public:
  FrameParser(): state(WAIT_START), length(0), idx(0), chk(0) {}

  // Call for each incoming byte. When a full frame is ready, sets outPayload and returns true.
