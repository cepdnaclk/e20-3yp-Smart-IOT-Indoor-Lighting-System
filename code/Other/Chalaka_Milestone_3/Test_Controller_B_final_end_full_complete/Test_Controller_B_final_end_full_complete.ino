...
static void onChunk(const String& envelope) {
  ...
  if (buf.received == buf.total) {
    ...
    // 7) ==== New Part: Re-chunk the full string and send over ESP-NOW ====

  //   const size_t MAX_SAFE_CHUNK = 100; // max safe size per ESP-NOW message
  //   uint32_t nowSeq = millis(); // use millis as unique ID
  ...
  //       Serial.printf("[WARN] Envelope too large for ESP-NOW (len=%u)\n", payload.length());
  //     } else {
  //       Serial.println(payload);
  //       Serial.printf("[ESP-NOW] Sending chunk %u/%u (len=%u)\n", i + 1, numChunks, payload.length());
  //       ESPNowManager::send(payload);
  //     }
  ...
  // 7) ==== Reliable Re-chunking and Sending ====

    const size_t MAX_SAFE_CHUNK = 100;
    uint32_t nowSeq = millis();
    uint16_t numChunks = (full.length() + MAX_SAFE_CHUNK - 1) / MAX_SAFE_CHUNK;
    ...
    for (uint16_t i = 0; i < numChunks; i++) {
      ...
      if (payload.length() <= 250) {
        ...
        Serial.printf("[WARN] Chunk %u too large, skipped\n", i);
      }
    }

    lastSendTime = millis(); // Start timer for first send
  }
}
...
void loop() {
  ...
  // Optional pacing
  // delay(10);
  //delay(10); // or tune between 5-50ms based on stability
