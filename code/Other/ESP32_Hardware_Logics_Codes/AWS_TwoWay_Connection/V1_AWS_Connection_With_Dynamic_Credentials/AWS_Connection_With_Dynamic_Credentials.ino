...
// Global WiFi/MQTT objects and constants
...
// Define the configuration trigger pin (GPIO0)
#define CONFIG_PIN 0
...
void resetCredentials() {
  ...
  preferences.clear(); // Removes all keys (e.g., SSID and password) in the "wifi" namespace
  preferences.end();
  Serial.println("WiFi credentials cleared from NVM.");
}

// Function to load stored WiFi credentials from NVM
...
// Function to save new WiFi credentials to NVM
void saveCredentials(String ssid, String password) {
  preferences.begin("wifi", false);
...
// Function to check if the config button (GPIO0) is pressed for 5 seconds
void checkConfigButton() {
  ...
  while (millis() - startTime < 5000) {
    if (digitalRead(CONFIG_PIN) == LOW) {
      ...
      startConfigPortal();
      // Note: startConfigPortal() blocks (or restarts after saving)
    }
    delay(100);
  }
}
...
// Updated setupWiFi() function using stored credentials
void setupWiFi() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
    if (millis() - startAttemptTime > checkInterval) {
...
void setup() {
  ...
  // Load stored WiFi credentials from NVM
  loadCredentials();
  
  // If no credentials are stored, start the configuration portal
  if (storedSSID == "" || storedPassword == "") {
    Serial.println("No WiFi credentials found.");
    startConfigPortal();
  }
  
  // Check if GPIO0 is pressed for 5 seconds to trigger reconfiguration
  checkConfigButton();
  
  // Now set WiFi to station mode and proceed
  WiFi.mode(WIFI_STA);
  setupWiFi();
  setupMQTT();
}
