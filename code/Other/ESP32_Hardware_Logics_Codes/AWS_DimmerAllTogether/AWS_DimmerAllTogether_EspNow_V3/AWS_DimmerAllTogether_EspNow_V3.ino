...
typedef struct {
    bool isAck;
    char jsonMessage[100];   // Stores the JSON message
    uint8_t senderMac[6];    // Stores sender MAC address
} EspNowMessage;

QueueHandle_t espNowQueue;

// Acknowledge message to sender
void sendAcknowledgment(const uint8_t *senderMac) {
    EspNowMessage ackMsg;
    memset(&ackMsg, 0, sizeof(ackMsg));
    ackMsg.isAck = true;
    esp_now_send(senderMac, (uint8_t*)&ackMsg, sizeof(ackMsg));
    Serial.println("📨 Sent Acknowledgment!");
}

// // ESP-NOW receive callback
// void onDataRecv_espnow(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
//     EspNowMessage msg;
//     memset(&msg, 0, sizeof(msg));
//     memcpy(&msg, incomingData, sizeof(msg));

//     if (msg.isAck) return; // Ignore acknowledgments

//     BaseType_t xHigherPriorityTaskWoken = pdFALSE;
//     xQueueSendFromISR(espNowQueue, &msg, &xHigherPriorityTaskWoken);
...
void onDataRecv_espnow(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
    EspNowMessage msg;
    memset(&msg, 0, sizeof(msg));
    memcpy(&msg.jsonMessage, incomingData, sizeof(msg.jsonMessage));
    memcpy(&msg.senderMac, mac_addr, 6); // Store sender MAC address
    ...
    sendAcknowledgment(msg.senderMac);

    Serial.print("Sender MAC: ");
    for (int i = 0; i < 6; i++) {
      if (msg.senderMac[i] < 16) {
        Serial.print("0");
      }
      Serial.print(msg.senderMac[i], HEX);
      if (i < 5) {
        Serial.print(":");
      }
    }
...
//   EspNowMessage msg;
//   memset(&msg, 0, sizeof(msg));
//   memcpy(&msg, incomingData, sizeof(msg)); // Fixed-size message assumed
//   BaseType_t xHigherPriorityTaskWoken = pdFALSE;
//   xQueueSendFromISR(espNowQueue, &msg, &xHigherPriorityTaskWoken);
//   if (xHigherPriorityTaskWoken) {
//     portYIELD_FROM_ISR();
...
void espNowTask(void * parameter) {
    ...
    for (;;) {
        if (xQueueReceive(espNowQueue, &receivedMsg, portMAX_DELAY) == pdTRUE) {
            ...
            Serial.println(receivedMsg.jsonMessage);

            DynamicJsonDocument doc(200);
            DeserializationError error = deserializeJson(doc, receivedMsg.jsonMessage);
            if (error) {
            ...
            Serial.printf("Bulb 1: %d\n", brightness1);
            Serial.printf("Bulb 2: %d\n", brightness2);
            Serial.printf("Bulb 3: %d\n", brightness3);
            Serial.printf("Bulb 4: %d\n", brightness4);

            // // sendAcknowledgment(mac_addr);
            // sendAcknowledgment(receivedMsg.senderMac);

            // Serial.print("Sender MAC: ");
            // for (int i = 0; i < 6; i++) {
            //   if (receivedMsg.senderMac[i] < 16) {
            //     Serial.print("0");
            //   }
            //    Serial.print(receivedMsg.senderMac[i], HEX);
            //   if (i < 5) {
            //   Serial.print(":");
            //   }
            // }
...
const int checkInterval = 60000; // 60 seconds
...
void setupWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Reinitialize ESP-NOW to use the AP's channel.
        esp_now_deinit();
        if (esp_now_init() != ESP_OK) {
            Serial.println("❌ ESP-NOW reinitialization failed!");
        } else {
            esp_now_register_recv_cb(onDataRecv_espnow);
            Serial.println("✅ ESP-NOW reinitialized and receive callback registered!");
        }
    } else {
...
void setup() {
    ...
    // Initialize ESP-NOW for ESP-NOW-only operations
    if (esp_now_init() != ESP_OK) {
        Serial.println("ESP-NOW Initialization Failed!");
        return;
    }
    // Register our prioritized callback.
    esp_now_register_recv_cb(onDataRecv_espnow);
    ...
    // Create queue and start high-priority ESP-NOW processing task.
    espNowQueue = xQueueCreate(10, sizeof(EspNowMessage));
    xTaskCreatePinnedToCore(espNowTask, "ESPNowTask", 2048, NULL, 3, NULL, 1);
